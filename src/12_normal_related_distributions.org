...
* Lognormal distribution
  ...
  $E(X)=e^{\theta+\omega^2/2}$ and $V(X)=e^{2\theta + \omega^2}\left( e^{\omega^2} - 1 \right)$
  #+reveal_html: </div>
  #+reveal: split
  #+attr_html: :width 75%
...
** Calculating lognormal probabilities
   ...
   $$=\dfrac{1}{2\pi} \int_{(\ln{a}-\theta)/\omega}^{(\ln{b}-\theta)/\omega} e^{-(1/2)[(s-0)/1]^2} ds$$
   $$= \Phi \left( \dfrac{\ln{b}-\theta}{\omega} \right) - \Phi \left( \dfrac{\ln{a}-\theta}{\omega} \right)$$
   #+reveal_html: </span>
** Example
...
** Example
   ...
   #+reveal_html: <span class="fragment">
   $$P(X \leq 150) = \sum_{x=0}^{150} \binom{6 \times 10^6}{x}(10^{-5})^x(1-10^{-5})^{16 \times 10^6 - x}$$
   #+reveal_html: </span>
